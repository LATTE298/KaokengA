...
รายการทั้งหมด **219 คลิป** ตรงกับ [`assets/tts/tts_manifest.json`](../assets/tts/tts_manifest.json) แล้ว ไม่ต้องแก้ JSON ใดๆ — แค่วางไฟล์ให้ชื่อตรง (คำศัพท์อิงคลังคำจริง 6 หมวด 90 คำจาก zip "NSC คำศัพท์ + ภาพ") — ชุดบังคับจริงๆ คือ 129 คลิปแรก ส่วน `match_*` 90 คลิปอัดทีหลังได้
...
### สถานการณ์ Module A (17)

ฉากเซเว่น (โหมดโจทย์ชิ้นเดียว) สุ่มเป้าหมายใหม่ทุกรอบ ประโยคสั่ง/ใบ้จึง**ประกอบจากชื่อไอเทม** ("น้องช่วยหยิบ_X_ใส่ตะกร้าให้หน่อยนะครับ" / "ลองหยิบ_X_นะครับ") — ต้องอัดครบทุกไอเทมของฉากนั้น
ฉากแยกขยะ (คัดแยกครบทุกชิ้น) ใช้ประโยคสั่ง/ใบ้ของฉากตรงๆ. ฉากจัดผลไม้ (สุ่มโจทย์ 2 ชนิด) ประกอบประโยคจากคู่ผลไม้ที่สุ่มได้ — 6 คู่ = 6 คลิป (ประโยคใบ้ตอนนิ่งนาน = ทวนประโยคโจทย์เดิม ไม่ต้องมีคลิปแยก). เสียงชม/ปลอบระหว่างเล่น reuse `sys_quiz_correct`/`sys_quiz_retry`

| ไฟล์ | ข้อความ |
|---|---|
| `sc_711_milk_title` | ช่วยหยิบของที่เซเว่น |
| `sc_trash_sort_title` | ช่วยแยกขยะให้ถูกถัง |
| `sc_food_prep_title` | ช่วยจัดผลไม้ใส่ถ้วย |
...
| `sc_sort_trash_ask` | น้องช่วยลากขยะแต่ละชิ้น ไปทิ้งถังให้ถูกต้องนะครับ |
| `sc_sort_trash_hint` | ดูรูปบนถังนะครับ ขยะเหมือนรูปไหน ทิ้งถังนั้นเลยครับ |
| `sc_fruit_pick_apple_grapes` | น้องช่วยหยิบแอปเปิ้ลกับองุ่นใส่ถ้วยให้หน่อยนะครับ |
| `sc_fruit_pick_apple_orange` | น้องช่วยหยิบแอปเปิ้ลกับส้มใส่ถ้วยให้หน่อยนะครับ |
| `sc_fruit_pick_apple_banana` | น้องช่วยหยิบแอปเปิ้ลกับกล้วยใส่ถ้วยให้หน่อยนะครับ |
| `sc_fruit_pick_grapes_orange` | น้องช่วยหยิบองุ่นกับส้มใส่ถ้วยให้หน่อยนะครับ |
| `sc_fruit_pick_grapes_banana` | น้องช่วยหยิบองุ่นกับกล้วยใส่ถ้วยให้หน่อยนะครับ |
| `sc_fruit_pick_orange_banana` | น้องช่วยหยิบส้มกับกล้วยใส่ถ้วยให้หน่อยนะครับ |

หมายเหตุ: `tts_celebration` ในไฟล์ฉากไม่ถูกใช้ตรงๆ (คำชมจบเกมสุ่มจาก `sys_celebration_1-3`) — ฉากเซเว่น/จัดผลไม้ไม่ใช้ `tts_instruction/hint` ใน JSON ด้วย (ประกอบสดจากไอเทมที่สุ่ม)
