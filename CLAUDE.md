...
- **⏸️ เชื่อม Firebase project ใหม่ `kaokeng-app` (เริ่ม 2026-07-07) — ค้างที่ขั้น 6/7 กลับมาทำต่อ:**
  ...
  🔜 **RESUME (ขั้น 7 — ด่านสุดท้าย, test จริง):** เหลือทดสอบ login อย่างเดียว: (ก) **web เร็ว** `flutter run -d chrome` → สมัคร/login email/password → เช็ค Firestore มี `users` doc (พิสูจน์เชื่อม Firebase สำเร็จ end-to-end); Google Sign-In บน web ต้องเพิ่ม meta clientId (ยังไม่ทำ) (ข) **Android เต็ม** `flutter run` → email/password + Google Sign-In (ต้องมี Android SDK/emulator/device — เดิมว่าเครื่อง dev ไม่มี SDK, เช็ค `flutter devices` ก่อน)
