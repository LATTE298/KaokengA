...
- **✅ เชื่อม Firebase project ใหม่ `kaokeng-app` (2026-07-07) — เชื่อมครบ + web login ผ่านแล้ว, เหลือทดสอบบน Android:**
  ...
  🔜 **RESUME (เหลือ):** (1) **ทดสอบบน Android จริง** — Google Sign-In เต็มรูปแบบ + orientation (เครื่อง dev ไม่มี SDK, `flutter devices` เห็นแค่ web/windows; Google บน web ต้องเพิ่ม meta `google-signin-client_id` ใน `web/index.html`, client_type 3 id อยู่ใน google-services.json) (2) **asset** — เจนเสียง 219 คลิป (`docs/tts_clips.csv` + `docs/TTS_CLIPS.md`), หมวดน้ำรูปการ์ตูน, รูปถ่านการ์ตูนแทน Energizer (3) **ฉากแยกขยะยังเป็นระบบเดิม (ยืดเต็มจอ)** — ถ้าอยากไม่ยืดบน iPad เหมือนฉากผลไม้ ให้ใส่ `cover_fit: true` + แปลง `zones`/พิกัดเป็นสัดส่วน 0..1 ของ `bg_trash_bins.jpg` (โซนถัง 4 ใบ) (4) การ์ดครอบครัวโหมดสุ่มโชว์ "ตัวเลือก: " ว่าง (gap 3 ใน memory)
  📌 origin/main อยู่ที่ `1b98e95` (2026-07-10, ฉาก sort-all) + กำลังเพิ่มก้อนฉากผลไม้ (cover-fit/responsive/polish) — ผมถูกบล็อก push main อัตโนมัติ ผู้ใช้ต้องสั่ง `git push origin main` เอง
