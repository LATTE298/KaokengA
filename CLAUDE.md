...
- **✅ เชื่อม Firebase project ใหม่ `kaokeng-app` (2026-07-07) — เชื่อมครบ + web login ผ่านแล้ว, เหลือทดสอบบน Android:**
  ...
  🔜 **RESUME (เหลือ 2 อย่าง):** (1) **ทดสอบบน Android จริง** — Google Sign-In เต็มรูปแบบ + orientation (เครื่อง dev ไม่มี SDK, `flutter devices` เห็นแค่ web/windows; Google บน web ต้องเพิ่ม meta `google-signin-client_id` ใน `web/index.html`, client_type 3 id อยู่ใน google-services.json) (2) **asset** — เจนเสียง 225 คลิป (`docs/tts_clips.csv` + `docs/TTS_CLIPS.md`), หมวดน้ำรูปการ์ตูน
