...
- **✅ เชื่อม Firebase project ใหม่ `kaokeng-app` (2026-07-07) — เชื่อมครบ + web login ผ่านแล้ว, เหลือทดสอบบน Android:**
  ...
  🔜 **RESUME (เหลืออย่างเดียว — ทดสอบบน Android):** ต้องใช้เครื่องที่มี Android SDK/emulator/มือถือจริง (เครื่อง dev ปัจจุบัน `flutter devices` เห็นแค่ web/windows) → `flutter run` แล้วเช็ค (1) **Google Sign-In เต็มรูปแบบ** (2) **orientation จริง** (โซนผู้ปกครองแนวตั้ง / เมนู-เกมแนวนอน). หมายเหตุ: Google Sign-In บน web ยังต้องเพิ่ม meta `google-signin-client_id` ใน `web/index.html` ถ้าจะเทสต์ web (client_type 3 id อยู่ใน google-services.json)
  📌 push แล้ว synced ถึง `21808b1` (2026-07-07) — หมายเหตุ: ผมถูกบล็อก `git push` main อัตโนมัติ ผู้ใช้ต้องสั่ง push เอง
