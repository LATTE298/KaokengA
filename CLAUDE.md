...
- **✅ เชื่อม Firebase project ใหม่ `kaokeng-app` (2026-07-07) — เชื่อมครบ + web login ผ่านแล้ว, เหลือทดสอบบน Android:**
  ...
  ✅ **ต่อมา (2026-07-14) ระบบรางวัล (A+C) + สลับรูปกาแฟ/สถานที่:** (a) **สลับ asset** — `coffee.jpg`=กาแฟNOBG, หมวดสถานที่ 15 รูปด้วย asset ใหม่ (ทับไฟล์ชื่อเดิม ไม่แตะ vocabulary.json — Flutter อ่าน PNG ในไฟล์ .jpg ได้) (b) **ระบบรางวัลฝั่งเด็ก** เข้าจากปุ่ม 🏆 ขวาบนหน้าเลือกเล่น (เลิก coming-soon) → `RewardsScreen` (ล็อกแนวนอน) 2 ส่วน: **สมุดสะสมสติกเกอร์** (ดาวสะสม `totalStarsProvider` ปลดทีละใบตามเกณฑ์ขั้นบันได `stickerThreshold`: 10/25/40/70/100/130 แล้ว +30 ต่อใบ [ผู้ใช้แก้ 2026-07-14, เดิม 10 คงที่], 24 ใบ emoji — ทีมสลับเป็นภาพวาดจริงได้โดยแก้แค่ `features/rewards/rewards_catalog.dart`) + **เหรียญความสำเร็จ 10 เหรียญ** (ปลดอัตโนมัติ: ดาว 10/50/100/200, เล่นเกมแรก, เล่น 10/50 ครั้ง, ครบทุกเกม 4 module, สตรีค 3/7 วัน — เหรียญยังไม่ได้โชว์แถบ x/y). **ปรัชญา: สะสมอย่างเดียว ไม่มีวันเสีย** (ไม่มีเอาดาวไปแลก) เหมาะกลุ่มเป้าหมาย (c) **โครง:** `features/rewards/rewards_catalog.dart` (pure: StickerDef/MedalDef + `stickersUnlockedCount`/`starsToNextSticker`/`medalUnlocked` — testable), `providers/rewards_provider.dart` (`rewardsStatsProvider` เก็บ `games_completed`+`modules_played` ใน Hive `app_prefs`, กัน box ปิด=no-op), ทุกเกม 4 จุด completion เรียก `recordCompletion(kModuleX)` คู่กับ award ดาว, **`best_streak`** เก็บใน `streakProvider` (เหรียญสตรีคไม่ล็อกกลับเมื่อขาดวัน) (d) **tests +19** (catalog 14 + provider 4 + widget render 1) → **157 เขียว**. ⚠️ **widget test สถานะ "ปลดล็อก" เทสต์ไม่ได้ใน flutter_tester** เพราะการ์ดปลดล็อกโชว์ emoji (🐱🏆) ที่ test runner ไม่มีฟอนต์ emoji สี → `pumpWidget`/`pumpAndSettle` **ค้าง 10 นาที** (Android จริงมี NotoColorEmoji ปกติ ไม่ใช่บั๊กแอป) — จึงคุมเฉพาะ logic ด้วย unit test + render สถานะว่าง. **บทเรียน: widget test ทุกตัวต้องตั้ง `GoogleFonts.config.allowRuntimeFetching = false` ใน setUpAll ไม่งั้น google_fonts โหลดเน็ตค้าง; และเลี่ยง emoji ใน widget ที่จะ pump ในเทสต์** (e) push ขึ้น main แล้ว (commit `31494d4` images + `ad9cd51` rewards)
  ✅ **ต่อมา (2026-07-14 ต่อ) เสียงดาว + Achievement toast + APK:** (a) **เสียง Right02 ตามจังหวะดาว** ใน `GameResultDialog` — แปลงเป็น `ConsumerStatefulWidget` + listener บน AnimationController เล่น `kSfxRight` ทีละครั้งตอนดาวแต่ละดวงเริ่มเด้ง (จังหวะ `_kStarPopBegin`/`_kStarPopStagger` = ค่าเดียวกับที่ `_StarsRow` ใช้วาด) เฉพาะดาวที่ได้จริง (b) **Achievement toast สไตล์ Steam/GPG** — `widgets/achievement_overlay.dart` ครอบ app root (main.dart builder ใน `BgmGate>AchievementOverlay>UsageTimerGate`): ฟังคิว `achievementQueueProvider` แล้วเด้ง toast สไลด์จากขอบบน คิวทีละใบ + IgnorePointer (ไม่ขวางกด) + เสียง **`kSfxAchievement`=`assets/sfx/achievement.mp3`** (Kaokeng Achievement, มีไฟล์จริงแล้ว) (c) **`awardGameResult(ref, module, stars)`** ใน rewards_provider — รวมศูนย์ บวกดาว+`recordCompletion`+ตรวจ `newlyUnlocked(before→after)` (pure ใน rewards_catalog)+enqueue notice; **แทนที่โค้ด award/record เดิมใน 4 หน้าเกม** (ตัด import `child_profile_provider` ที่ไม่ใช้แล้วออกจากทั้ง 4) (d) ⚠️ **บั๊กที่เจอ+แก้:** `AchievementOverlay` เดิมสร้าง `_c` แบบ `late final` (init ตอนแตะครั้งแรก) — ถ้าไม่เคยมี toast เด้งเลย `_c` จะถูกสร้างครั้งแรก **ตอน `dispose()`** → lookup TickerMode ของ element ที่ deactivate แล้ว = crash → ทำ unmount ค้าง timer อื่น (UsageTimer/BGM) ไม่ถูก cancel → `widget_test.dart` fail. **แก้: ย้ายสร้าง `_c`/`_anim` ไป `initState`**. บทเรียน: AnimationController ที่ dispose แน่ๆ ต้องสร้างใน initState ไม่ใช่ lazy (e) **tests +8** (newlyUnlocked 4 + achievement queue 4) เขียว; ยืนยัน analyze + vocab completion + `widget_test` app-root ผ่าน exit สะอาด (f) push แล้ว (`a7954f8` เสียงดาว + `6d9ed95` achievement) → building APK. ⚠️ **flutter test บน Windows ไม่ยอม exit หลัง test hang/timeout** — เคยค้าง background shell 1.5 ชม.; ให้รันด้วย OS guard `timeout 120 flutter test ... --timeout=30s` เสมอ กันค้างยาว
  ✅ **ต่อมา (2026-07-14 ต่อ2) แก้ toast + ลบพื้นหลังสินค้า + ตัด TTS-gen:** (a) **บั๊ก underline เหลืองใต้ข้อความ toast** — `Text` ใน AchievementOverlay ไม่มี `Material` ancestor (ลอยเหนือ Scaffold) → Flutter วาดสไตล์ error ขีดเส้นใต้เหลืองคู่. **แก้: ครอบ `Material(type: MaterialType.transparency)`** ใน `_AchievementCard` (บทเรียน: widget ที่โชว์ Text นอก route/Scaffold ต้องมี Material ครอบ) (b) **ลบพื้นหลังขาว+เงา สินค้า 3 ชิ้นในฉากเซเว่น** (`milk_blue.webp`/`bread.webp`/`chips.webp` — จริงๆ เป็น JPEG 1024² ในชื่อ .webp) ด้วย flood-fill จากขอบ (C# System.Drawing, เกณฑ์ bg = bright≥200 & sat≤30 → ลบขาว+เงาเทา แต่เก็บขาวในตัวสินค้าที่ถูกล้อมด้วยเส้นขอบเข้ม) เซฟเป็น PNG RGBA ทับชื่อ .webp เดิม (Flame decode ตาม content ไม่ต้องแก้ JSON) — สคริปต์ `scratchpad/cutout_products.ps1` (c) ตัด TTS-gen (ดู RESUME) — analyze สะอาด + `widget_test`/`assets_test` ผ่าน (ยังไม่ push ณ ตอนเขียน)
  ✅ **ต่อมา (2026-07-14 ต่อ3) เสียงจบด่าน + ปุ่มเล่นอีกครั้ง:** (a) **เสียง `kSfxCongrat`=`assets/sfx/congrat.mp3`** (Kaokeng_congrat) เล่น **ทันทีที่จบด่าน ไม่ใช่ตอน popup** (ผู้ใช้แก้ 2026-07-14 หลายรอบ): **การ์ด (memory/vocab/family)** = ที่จุด `result.completed`/`_onComplete` ก่อนหน่วง 1.2วิ+popup. **Module A (ชีวิตประจำวัน)** = ใน `daily_life_game._handleSuccess` ตรงจุด `add(SuccessOverlayComponent)` = **พร้อมเอฟเฟกต์ confetti วงกลมฟ้า-เหลือง** (ไม่ใช่ที่ `onComplete` ของหน้าจอ ซึ่งเกิด 2.5วิ หลัง confetti). ⚠️ ลำดับใน _handleSuccess: `await tts.speak(celebration)` → `sfx.play(kSfxCongrat)` + confetti → delay 2.5วิ → onComplete/popup (b) **ปรับ confetti สวยขึ้น** (`success_overlay.dart`): 110 ชิ้น + พาเลตต์ 6 สีโทนแบรนด์ + `ComputedParticle` จาง/ย่อขนาดนุ่มช่วงท้าย + ผสมวงทึบ/วงขอบ(ring)/วงใหญ่มีไฮไลต์ขาว + น้ำพุพุ่งขึ้นตกตามแรงโน้มถ่วง (b) **ปุ่ม "เล่นอีกครั้ง"** ใน dialog: เพิ่ม `onPlayAgain` (ไม่ null = โชว์ 2 ปุ่ม "ปิด"(รอง)+"เล่นอีกครั้ง"(เด่น)). ทุกเกมส่ง onPlayAgain = **pop หน้าเกมเดิม → push เส้นทางเดิมใหม่** (GoRouter object, ได้เกมรอบใหม่/สุ่มใหม่; memory กลับไปเลือกระดับ). scenario รื้อ `_showResultDialog` helper มา inline (ตัด pop ท้าย) ⚠️ **บทเรียน: อย่าเรียก `GoRouter.of(context)` แบบ eager ก่อน showDialog** — vocab widget test pump ใน `MaterialApp` ธรรมดา (ไม่มี GoRouter) → throw "No GoRouter found". ย้ายเข้า callback (lazy, รันตอนกดปุ่มเท่านั้น) + เก็บ router object ไว้ก่อน pop (context หน้าเกม defunct หลัง pop) — **164 tests เขียว**
  ✅ **ต่อมา (2026-07-14 ต่อ4) แก้บั๊กเสียง/โจทย์ไม่ตรงกันตอน "เล่นอีกครั้ง" (Module A):** `ScenarioGameScreen` เดิมเป็น `ConsumerWidget` เรียก `_randomizeTarget()` (ใช้ `Random()`) + สร้าง `DailyLifeGame` **ใน `build()`** → ทุก build สุ่ม target ใหม่ + game ใหม่ (leak) ทำให้เสียงพากย์ (game พูด ttsInstruction) กับแถบโจทย์ (targetItem) เพี้ยนกันเมื่อ build ซ้ำ (เห็นชัดตอน replay). **แก้: แปลงเป็น `ConsumerStatefulWidget` แล้ว cache `_randomized`/`_game` ด้วย `??=`** (สุ่ม+สร้างครั้งเดียวต่อ instance; replay = State ใหม่ = สุ่มใหม่แต่เสถียร). บทเรียน: **อย่าใช้ `Random()`/สร้าง object หนักใน `build()`** ของ widget ที่ rebuild ได้
  ✅ **ต่อมา (2026-07-14 ต่อ5) เกมซื้อของโหมดใหม่ "shop" (นับจำนวนหลายชนิด):** ฉาก `711_milk_001` เปลี่ยนจากหยิบชิ้นเดียว → **pool 10 ชิ้น** (3 เดิม + 7 จากรูป vocab อาหาร: candy/pizza/burger/sushi/boiled_egg/pie/fried_chicken — อ้าง path `assets/images/vocab/*.png` ตรงๆ ใน JSON) **สุ่มโชว์ 3**, โจทย์ **1-2 ชนิด รวม ≤4 ชิ้น** (เช่น "นม 3 กล่อง" / "ขนม 2 + นม 1"). ของบนชั้น**ลากซ้ำได้** (ลากลงตะกร้าถูก→เด้งกลับชั้น+นับ), ผิด/เกินจำนวน→เด้งกลับ (mistake). **โครง:** schema เพิ่ม `shop_mode`+`display_count` (Freezed regen), `DailyLifeGame` โหมด `isShop`: `_generateOrder`/`_shopAccepts`/`_handleShopItem`/`_loadShopContent` + `basketNotifier` (ValueNotifier แจ้ง UI), `DropZoneComponent` เพิ่ม `acceptCheck`/`multiAccept`/`returnItems`, `InteractableComponent.returnToShelf()`, l10n เพิ่มชื่อ 7 ไอเทม + ลักษณนาม (`scenarioItemCounterTh`) + `shopOrderAsk`, แถบโจทย์ `_ShopObjectiveBar` (ValueListenableBuilder โชว์ "นม 1/3" เขียวเมื่อครบ). ⚠️ `ValueListenable` ต้อง import foundation ตรงๆ (ชนกับ flame export). tests +1 (`daily_life_game_sort_test`), แก้ render/assets test รองรับ shop → **170 เขียว**. **แก้รอบ on-device (2026-07-15):** (1) **ข้อความโจทย์หาย** — `shopOrder` สร้างใน onLoad (async) แต่แถบโจทย์ Flutter build ก่อน → ได้ list ว่าง. **ย้าย `_initShop()` (เลือกของ+สร้างโจทย์) ไป constructor** (2) **ของลอยไม่ตรงชั้น** — เดิมวาง `size.y*0.60` (screen). เปลี่ยนเป็น authoring space (`_toCanvas`, y=700 ชั้นเดียวกับตะกร้า, x 300..1150 ซ้ายของตะกร้า) — **อาจต้อง nudge y ต่อ** (3) **ตะกร้าเป็นกรอบเหลือง** → ใส่รูปจริง `assets/images/basket.png` (basketNOBG) เป็น SpriteComponent contain-fit + DropZone `visible:false`. **แก้เพิ่ม 2026-07-15b:** ผู้ใช้ให้ **กลับเป็น 3 ชิ้นเดิมเท่านั้น** (milk/bread/chips — เอา 7 vocab ออกจาก JSON, ยังคง shop_mode/quantity), ของวางตำแหน่งเดิม **x 300/600/900, y=500** (700 ก่อนหน้าต่ำไป ของล้นหน้าชั้น), ตะกร้าขยาย 1.4 เท่า (sprite+hitbox รอบ center เดิม). กลไก quantity (นับหลายชนิด) คงไว้ — pool=3 → โชว์ทั้ง 3 ทุกรอบ สุ่มแค่โจทย์จำนวน/ชนิด
  🔜 **RESUME (เหลือ):** (1) **ทดสอบบน Android จริง** — Google Sign-In เต็มรูปแบบ + orientation (เครื่อง dev ไม่มี SDK; web meta ใส่แล้ว) (2) **asset** — ~~เจนเสียง 219 คลิป~~ **DROPPED โดยผู้ใช้ 2026-07-14** (ใช้ TTS ในแอปไปเลย — Cloud TTS/เสียง engine ในเครื่อง; ระบบ BundledTtsClient/manifest ยังอยู่เผื่ออนาคตแต่ไม่ต้องเจนแล้ว), หมวดน้ำรูปการ์ตูน, รูปถ่านการ์ตูนแทน Energizer, **avatar หน้าเลือกเล่น `home_avatar.png`** (ตอนนี้ fallback ไอคอน) (3) ✅ **ต่อ provider หน้าเลือกเล่นแล้ว (2026-07-12)** — ชื่อเด็ก/ดาวสะสม/ความคืบหน้าเป็นของจริง เหลือแค่เมนู "รางวัล" ที่ยัง coming-soon (ผูกเฟส 3.1 พักรอ design) + avatar `home_avatar.png` ยัง fallback ไอคอน (4) การ์ดครอบครัวโหมดสุ่มโชว์ "ตัวเลือก: " ว่าง (gap 3 ใน memory) (5) **วิดีโอพื้นหลัง `kaokeng_bg.mp4` = 4.45MB (BackgroundAnimated5, สลับ 2026-07-12)** — สลับได้โดยเขียนทับไฟล์ชื่อเดิม ไม่ต้องแก้โค้ด (loop/fallback/BGM คงเดิม); เครื่องนี้ไม่มี ffmpeg ถ้าจะบีบต้องใช้เครื่องอื่น
