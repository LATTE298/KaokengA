...
- **✅ เชื่อม Firebase project ใหม่ `kaokeng-app` (2026-07-07) — เชื่อมครบ + web login ผ่านแล้ว, เหลือทดสอบบน Android:**
  ...
  🔜 **RESUME (เหลือ):** (1) **ทดสอบบน Android จริง** — Google Sign-In เต็มรูปแบบ + orientation (เครื่อง dev ไม่มี SDK; web meta ใส่แล้ว) (2) **asset** — เจนเสียง 219 คลิป, หมวดน้ำรูปการ์ตูน, รูปถ่านการ์ตูนแทน Energizer, **avatar หน้าเลือกเล่น `home_avatar.png`** (ตอนนี้ fallback ไอคอน) (3) **หน้าเลือกเล่น placeholder ที่เหลือต่อ provider จริง:** ชื่อเด็ก/ดาว (`_kChildName`/`_kStarCount` คงที่), เมนู "รางวัล"/"ความคืบหน้า" (เด็ก) ยัง coming-soon (4) การ์ดครอบครัวโหมดสุ่มโชว์ "ตัวเลือก: " ว่าง (gap 3 ใน memory) (5) **วิดีโอพื้นหลังใหญ่ไป?** — 1.6MB โอเค; ถ้าเปลี่ยนตัวใหญ่ (เช่น .mov 21MB) ต้องบีบก่อน (เครื่องนี้ไม่มี ffmpeg)
  📌 origin/main sync ถึง `1d07bd7` (2026-07-12, กระดาษครีม) + กำลังเพิ่มก้อน BGM+ปุ่มครอบครัว — push main ผู้ใช้สั่ง `git push origin main` เอง (ก้อนถังขยะ/SFX/ธีมใหม่/สตรีค commit ไปแล้ว 8321e94)
