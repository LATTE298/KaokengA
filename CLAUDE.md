...
- **Module C (คำศัพท์):** hub 2 โหมด — **sound board** (แตะรูป→ฟังเสียง) + **เกมตอบคำถาม** (โชว์รูปใหญ่→ถาม "นี่คือ...อะไร?" ตามหมวด→เลือกปุ่ม ก/ข/ค) ✅ ตรง mockup ในเอกสารข้อเสนอแล้ว. Scoring: ตอบผิดรวม 0=10, 1=8, 2=6, ≥3=4
...
- **โครงสร้าง:** `features/vocab_quiz/vocab_quiz_controller.dart` (logic ล้วน testable แบบเดียวกับ memory) — สุ่ม 5 ข้อจาก 30 คำ ไม่ซ้ำ, ตัวเลือก 3 ตัว ตัวลวงหมวดเดียวกัน, ตอบผิด → ล็อกปุ่มนั้นแล้วลองต่อจนถูก (เด็กจบทุกข้อด้วยความสำเร็จเสมอ ไม่มี "แพ้"). UI ที่ `screens/child/vocab_quiz_screen.dart` **ตาม mockup เอกสาร**: ซ้าย=รูปโจทย์ใหญ่ (แตะฟังคำถามซ้ำ) + "คำถาม: นี่คือสัตว์อะไร?", ขวา=ปุ่มคำตอบ ก/ข/ค (responsive ตามกฎข้อ 3)
...
4. **รูปเกม Module A จาก zip ยังไม่ได้ต่อเข้าเกม** — ทีมส่งการ์ดขยะ 4 ประเภท+ถังสี่ใบ (ฉากแยกขยะ) และผลไม้ 5 ชนิด+ถ้วย (ฉากจัดจาน) มาแล้ว (อยู่ที่ scratchpad `nsc_assets/ภาพเกมชีวิตประจำวัน/`) — ใช้อัปเกรดฉาก trash_sort/food_prep ให้เป็นรูปจริงได้ ต้องปรับ scenario JSON + ลบ placeholder ที่เกี่ยวข้อง
